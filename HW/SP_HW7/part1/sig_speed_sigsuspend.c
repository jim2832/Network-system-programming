...
main(int argc, char *argv[])
{
    ...
    case 0:     /* child */
        for (int scnt = 0; scnt < numSigs; scnt++) {
            if (kill(getppid(), TESTSIG) == -1) // getppid() returns the parent's PID
                errExit("kill");
            if (sigsuspend(&emptyMask) == -1 && errno != EINTR) // sigsuspend() suspends the process until a signal is caught
                    errExit("sigsuspend");
        }
        exit(EXIT_SUCCESS);
